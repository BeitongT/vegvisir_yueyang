...
import edu.cornell.em577.tamperprooflogging.API.DagCallback
...
class BlockRepository private constructor(private val env: Pair<Context, Resources>) {
    ...
    companion object :
        ...
        private const val BoomerRadix=8
    }
    ...
    private val registeredCallbacks = HashSet<DagCallback>()
...
//        updateBlockChain(listOf(genesisBlock), genesisBlock)
//        updateBlockChain(listOf(userCertBlock), userCertBlock)
    ...
    private fun blockChecking(){
        val existHashes=HashSet<String>(listOf(ROOT))
        val queue = ArrayDeque<SignedBlock>(listOf(signedBlockByCryptoHash[ROOT]!!))
        while (queue.isNotEmpty()) {
            val currentBlock = queue.pop()
            existHashes.add(currentBlock.cryptoHash)
        ...
        val hashesNeedRemove=signedBlockByCryptoHash.keys-existHashes
        for(hash in hashesNeedRemove)
        {
            blockstore.getDocument(hash).delete()
            signedBlockByCryptoHash.remove(hash)
        }
    }

    // Update BoomerFilter in the reverse BFS order of the blockchain
    // ugly implementation
    private fun updateBoomer() {
        val queue = ArrayDeque<SignedBlock>(listOf(signedBlockByCryptoHash[ROOT]!!))
        val stack = ArrayDeque<SignedBlock>(listOf(signedBlockByCryptoHash[ROOT]!!))
        while (queue.isNotEmpty()) {
            val currentBlock = queue.pop()
            for (parentHash in currentBlock.unsignedBlock.parentHashes) {
                val parentBlock = signedBlockByCryptoHash[parentHash]!!
                queue.add(parentBlock)
                stack.add(parentBlock)
            }
        }
        while(stack.isNotEmpty()){
            boomer.addBlock(stack.pollLast())
        }
    }

    // Update ChainSpliter in the reverse BFS order of the blockchain
    // ugly implementation
    private fun updateSpliter(){
        val queue = ArrayDeque<SignedBlock>(listOf(signedBlockByCryptoHash[ROOT]!!))
        val stack = ArrayDeque<SignedBlock>(listOf(signedBlockByCryptoHash[ROOT]!!))
        while (queue.isNotEmpty()) {
            val currentBlock = queue.pop()
        ...
        while(stack.isNotEmpty()){
            spliter.addBlock(stack.pollLast())
        }
    }
    ...
    private fun populateRepos() {
        ...
        val queue = ArrayDeque<SignedBlock>(listOf(rootBlock))
...
//            populateReposWithTransactions(transactions)
    ...
    private fun addBlock(block: SignedBlock) {
        ...
        spliter.addBlock(block)
        //emit(block.cryptoHash)
    }
    ...
    private fun emit(hash: String) {
        synchronized(registeredCallbacks)
        {
            for (cb in registeredCallbacks) {
                cb?.OnNewBlockAdded(hash)
            }
        }
    }
}